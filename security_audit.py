...
class CommandExecutor:
    ...
    @staticmethod
    def run_command(
        command: Union[str, List[str]], timeout: int = 30, check_return_code: bool = False
        ...
        try:
            ...
            return {"command": command, "error": str(e), "success": False, "returncode": -1}
...
class SecurityAuditor:
    ...
    def __init__(self, verbose: bool = False):
        ...
        self.compliance_status = {"cis_controls": [], "nist_framework": []}
    ...
    def print_header(self) -> None:
        ...
        print(
            f"  Environment: {'Virtual Machine' if self.system_info['is_vm'] else 'Physical/Bare Metal'}"
        )
    ...
    def audit_fail2ban(self) -> None:
        ...
        if status_result["success"] and status_result["stdout"] == "active":
                ...
                for line in jail_lines:
                    if "Jail list:" in line:
                        ...
                        if "sshd" in jails:
                            ...
                            self.compliance_status["cis_controls"].append(
                                "CIS 6.2 - SSH Protection Active"
                            )
                            ...
                            self.print_finding(
                                "FAIL", "SSH jail not configured - CRITICAL SECURITY GAP"
                            )
                        ...
                        if "recidive" in jails:
                            self.print_finding(
                                "PASS", "Recidive jail configured for persistent attackers"
                            )
                        else:
            ...
            self.print_finding(
                "FAIL", "Fail2ban is not installed or not running - CRITICAL VULNERABILITY"
            )
            self.recommendations.append(
                {
                    "priority": "CRITICAL",
            ...
            self.compliance_status["cis_controls"].append(
                "CIS 6.2 - MISSING: Host-Based Intrusion Prevention"
            )
    ...
    def audit_firewall(self) -> None:
        ...
        if firewalld_status["success"] and firewalld_status["stdout"] == "active":
            ...
            if default_zone["success"]:
                ...
                if zone in ["drop", "block"]:
                    ...
                    print(
                        f"    {Colors.GREEN}🛡️  Security Level: Maximum (deny by default){Colors.END}"
                    )
                elif zone == "public":
                    self.print_finding("WARN", "Using 'public' zone - moderate security")
                    print(
                        f"    {Colors.YELLOW}⚠️  Security Level: Moderate (some services allowed){Colors.END}"
                    )
                    self.recommendations.append(
                        {
                            "priority": "MEDIUM",
            ...
            if log_denied["success"]:
                if log_denied["stdout"] != "off":
                    ...
                    print(
                        f"    {Colors.RED}🚨 Impact: Cannot detect attack patterns or compliance violations{Colors.END}"
                    )
                    self.recommendations.append(
                        {
                            "priority": "HIGH",
            ...
            if ufw_status["success"] and ufw_status["stdout"] == "active":
                ...
                iptables_rules["success"]
                and iptables_rules["stdout"].isdigit()
                and int(iptables_rules["stdout"]) > 10
            ):
                ...
                print(
                    f"    {Colors.RED}🚨 Impact: System completely exposed to network attacks{Colors.END}"
                )
                self.recommendations.append(
                    {
                        "priority": "CRITICAL",
        ...
        self.findings["firewall"] = {
            "firewalld_active": firewalld_status["success"]
            and firewalld_status["stdout"] == "active"
        }
    ...
    def _check_kernel_parameters(self) -> None:
        ...
        for param, (expected_value, description) in security_params.items():
            ...
            if result["success"]:
                ...
                if current_value == expected_value:
                    ...
                    self.print_finding(
                        "WARN",
                        f"{description} - current: {current_value}, expected: {expected_value}",
                    )
                    self.recommendations.append(
                        {
                            "priority": "MEDIUM",
        ...
        print(
            f"    {Colors.CYAN}🔧 Kernel hardening: {secure_params}/{total_params} ({security_percentage:.0f}%){Colors.END}"
        )
    ...
    def _check_system_services(self) -> None:
        ...
        if not active_risky_services:
            ...
            print(
                f"    {Colors.YELLOW}⚠️  {len(active_risky_services)} risky service(s) active{Colors.END}"
            )
    ...
    def _check_critical_file_permissions(self) -> None:
                ...
                if stat_result["success"]:
                    ...
                    if expected_perms == "000":  # Special case for shadow files
                        if current_perms in ["000", "640", "600"]:
                            ...
                            self.print_finding(
                                "WARN", f"{description} permissions too open: {current_perms}"
                            )
                        ...
                        self.print_finding(
                            "WARN",
                            f"{description} permissions: {current_perms} (expected: {expected_perms})",
                        )
                        self.recommendations.append(
                            {
                                "priority": "HIGH",
        ...
        print(
            f"    {Colors.CYAN}🔐 File security: {secure_files}/{len(critical_files)} files properly secured{Colors.END}"
        )
    ...
    def _check_privileged_users(self) -> None:
        ...
        if passwd_result["success"]:
            ...
            if len(uid_zero_users) == 1 and uid_zero_users[0] == "root":
                ...
                self.print_finding(
                    "WARN", f"Multiple UID 0 users detected: {', '.join(uid_zero_users)}"
                )
                self.recommendations.append(
                    {
                        "priority": "HIGH",
        ...
        if sudo_result["success"]:
            ...
            if sudo_users:
                ...
                print(
                    f"    {Colors.CYAN}🔑 {len(set(sudo_users))} user(s) have administrative privileges{Colors.END}"
                )
            else:
    ...
    def _check_inactive_accounts(self) -> None:
        ...
        if last_result["success"]:
            recent_logins = len(
                [
                    line
                    for line in last_result["stdout"].split("\n")
                    if line and "wtmp begins" not in line
                ]
            )
            self.print_finding(
                "INFO", f"Recent login activity detected for {recent_logins} sessions"
            )
    ...
    def _analyze_ssh_config(self, config_path: str) -> None:
        ...
        try:
            ...
            if "PasswordAuthentication no" in config_content:
                self.print_finding(
                    "PASS", "Password authentication disabled - using key-based auth"
                )
            ...
            if port_match:
                ...
                if port != "22":
                    ...
                    print(
                        f"    {Colors.YELLOW}💡 Consider changing to non-standard port{Colors.END}"
                    )
    ...
    def _analyze_ssh_connections(self) -> None:
        ...
        auth_log = self.executor.run_command(
            "journalctl -u sshd --since='24 hours ago' | grep -i 'authentication failure' | wc -l"
        )
        if auth_log["success"] and auth_log["stdout"].isdigit():
            failed_attempts = int(auth_log["stdout"])
            if failed_attempts > 0:
                self.print_finding(
                    "WARN", f"SSH authentication failures in last 24h: {failed_attempts}"
                )
    ...
    def _analyze_firewall_services(self) -> None:
        ...
        if services_result["success"]:
            ...
            print(
                f"    {Colors.CYAN}🔓 Allowed services: {', '.join(services) if services else 'None'}{Colors.END}"
            )
        ...
        if ports_result["success"] and ports_result["stdout"]:
            ...
            if len(ports) > 5:
                self.print_finding(
                    "WARN", f"Many open ports detected ({len(ports)}) - review necessity"
                )
        else:
    ...
    def _calculate_security_metrics(self) -> Dict[str, Union[int, float, str]]:
        ...
        overall_score = max(
            0, total_possible_score - critical_penalty - high_penalty - medium_penalty
        )
    ...
    def _display_enhanced_summary(self, metrics: Dict[str, Union[int, float, str]]) -> None:
        ...
        print(
            f"{Colors.BOLD}📊 Security Score: {score_color}{score}/100 {score_icon} ({risk_level} RISK){Colors.END}"
        )
        ...
        print(
            "  • Security best practices: https://docs.fedoraproject.org/en-US/quick-docs/securing-fedora/"
        )
    ...
    def _offer_interactive_remediation(self) -> None:
        ...
        print(
            f"  {Colors.GREEN}1{Colors.END} - 📋 View detailed remediation steps (with follow-up options)"
        )
        ...
        try:
            ...
            if choice == "1":
                ...
                print(
                    f"{Colors.BLUE}ℹ️  Remediation skipped. Run audit again after manual fixes.{Colors.END}"
                )
            else:
    ...
    def _generate_fix_script(self) -> None:
        ...
        try:
            ...
            print(
                f"{Colors.YELLOW}⚠️  Review the script before running: cat {script_filename}{Colors.END}"
            )
    ...
    def _continue_after_hardening_suggestion(self) -> None:
        ...
        try:
            ...
            if choice == "1":
                ...
                print(
                    f"{Colors.YELLOW}💡 Tip: The hardening script provides comprehensive security improvements{Colors.END}"
                )
            else:
    ...
    def _execute_hardening_script(self) -> None:
        ...
        if not os.path.exists("security_hardening.sh"):
            print(
                f"{Colors.RED}❌ Security hardening script not found: security_hardening.sh{Colors.END}"
            )
            print(f"{Colors.YELLOW}💡 Make sure the script is in the current directory{Colors.END}")
            self._continue_after_hardening_suggestion()
            return

        print("The hardening script will:")
        ...
        try:
            confirm = (
                input(f"\n{Colors.YELLOW}Launch the hardening script? (y/N): {Colors.END}")
                .strip()
                .lower()
            )
            ...
            if confirm in ["y", "yes"]:
                ...
                try:
                    ...
                    print(
                        f"{Colors.BLUE}💡 Run the security audit again to see improvements{Colors.END}"
                    )
    ...
    def _continue_remediation_options(self) -> None:
        ...
        try:
            ...
            if choice == "1":
                ...
                print(
                    f"{Colors.YELLOW}💡 Tip: Run the audit again after applying fixes to see your improved security score!{Colors.END}"
                )
            else:
    ...
    def _apply_specific_fix(self) -> None:
        ...
        if not self.recommendations:
            print(
                f"{Colors.YELLOW}No recommendations available for interactive fixing.{Colors.END}"
            )
            return
        ...
        try:
            choice = input(
                f"\n{Colors.CYAN}Enter issue number to fix (0-{len(self.recommendations)}): {Colors.END}"
            ).strip()
    ...
    def _execute_specific_fix(self, issue_index: int) -> None:
        ...
        try:
            confirm = (
                input(f"\n{Colors.YELLOW}Do you want to apply this fix? (y/N): {Colors.END}")
                .strip()
                .lower()
            )
            ...
            if confirm in ["y", "yes"]:
                if fix.startswith("sudo ") or fix.startswith("echo "):
                    ...
                    print(
                        f"{Colors.YELLOW}⚠️  This fix requires manual configuration. Please apply it manually.{Colors.END}"
                    )
                    print(f"Command: {fix}")
            else:
    ...
    def _execute_generated_script(self, script_filename: str) -> None:
        ...
        try:
            confirm = (
                input(
                    f"\n{Colors.YELLOW}Are you sure you want to execute the script? (yes/NO): {Colors.END}"
                )
                .strip()
                .lower()
            )
            ...
            if confirm == "yes":
                ...
                try:
                    ...
                    if result["success"]:
                        ...
                        print(
                            f"{Colors.BLUE}💡 Recommendation: Run the security audit again to verify improvements{Colors.END}"
                        )

                        # Ask if they want to run audit again
                        rerun = (
                            input(
                                f"\n{Colors.CYAN}Run security audit again now? (y/N): {Colors.END}"
                            )
                            .strip()
                            .lower()
                        )
                        if rerun in ["y", "yes"]:
                            ...
                            print(
                                f"{Colors.BLUE}✅ Fixes applied. Run audit manually when ready.{Colors.END}"
                            )
                            self._continue_after_script_generation(script_filename)
                    else:
                        print(
                            f"{Colors.RED}❌ Script execution failed: {result['stderr']}{Colors.END}"
                        )
                        self._continue_after_script_generation(script_filename)
    ...
    def run_audit(self) -> None:
        ...
        if self.system_info["distribution"] == "unknown":
            self.print_finding(
                "WARN", "Unknown distribution detected - some checks may not work correctly"
            )
        ...
        print("  🔍 Contains: Detailed findings, remediation steps, compliance mapping")
    ...
    def _restart_audit_after_fixes(self) -> None:
        ...
        try:
            ...
            if self.recommendations:
                ...
                print(
                    f"{Colors.BLUE}💡 Your system security score should now be significantly improved.{Colors.END}"
                )
            ...
            print(
                f"{Colors.YELLOW}Please run the audit manually: python3 security_audit.py{Colors.END}"
            )
    ...
    def _show_improvement_summary(self) -> None:
        ...
        print(
            f"📊 Updated Security Score: {score_color}{current_score}/100 {score_icon} ({current_risk} RISK){Colors.END}"
        )
    ...
    def _offer_final_options(self) -> None:
        ...
        try:
            ...
            if choice == "1":
                ...
                print(
                    f"{Colors.BLUE}💡 Remember to run regular security audits to maintain security posture.{Colors.END}"
                )
            else:
...
Examples:
  ...
  %(prog)s --help             # Show this help message
    ...
    parser.add_argument(
        "--version", action="version", version="Fedora Security Hardening Toolkit v1.0.0"
    )
    ...
    if os.geteuid() != 0:
        print(
            f"{Colors.YELLOW}⚠️  Some checks require root privileges. Run with sudo for complete audit.{Colors.END}"
        )
